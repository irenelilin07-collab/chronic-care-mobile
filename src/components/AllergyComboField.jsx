...
import { searchAllergies } from "../lib/allergyCatalog.js";
import { ComboDropdownPanel, DropdownTable } from "./ComboDropdownTable.jsx";

export default function AllergyComboField({ label, placeholder, value, onChange, onPick }) {
  ...
  const rows = open
    ? searchAllergies(value).map((name) => ({ key: name, label: name, value: name }))
    : [];
